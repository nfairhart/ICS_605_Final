...
Results are summarized in Tables~\ref{tab:results} and \ref{tab:errors} and Figure~\ref{fig:metrics}. Fine-tuning improved score agreement from 49\% to 75\% and classification accuracy from 64\% to 87\%, representing gains of 26 and 23 percentage points respectively. Mean absolute error on match score decreased from 13.49 to 9.11 points. Both models exhibited a consistent positive scoring bias: virtually no predictions fell below the ground truth score, and mean signed error equaled MAE for both models, indicating the models over-predict rather than under-predict across the board.
...
\begin{figure}[h]
  ...
  \caption{Distribution of signed score error (predicted minus ground truth). Both models over-predict consistently; the fine-tuned model's errors are tighter (MAE 9.11) while the base model's are more dispersed (MAE 13.49).}
  \label{fig:histogram}
\end{figure}
